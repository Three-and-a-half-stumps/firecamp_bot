...
class TgPublicGroupOrChannelValidator(Validator):
  ...
  def __init__(self, error: str = None):
    self.error = error or (
      ...
      P('@channel_or_group_login', types='code') + P(' или ') +
...
class IntegerListValidator(Validator):
  ...
  def __init__(self, error: str = None):
    self.error = error or (P(
      'Что-то не так :( ожидается список чисел '
      '— просто числа, отделённые друг от друга '
      'пробелом и ничем больше. Например:\n',
      emoji='fail') + P('4 5 28 2', types='code'))
...
class WordListValidator(Validator):
  ...
  def __init__(self, error: str = None):
    self.error = error or (
      P(
        'Что-то не так :( ожидается список слов; '
