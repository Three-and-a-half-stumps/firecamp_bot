...
class Pieces:
  ...
  def getEntities(self) -> List[MessageEntity]:
    ...
    types = reduce_list(lambda a, b: a | b, [p.types for p in self.pieces],
                        set())
    entities = reduce_list(lambda a, b: a + b,
...
def P(text: str = None,
  ...
  if isinstance(types, str):
    types = [types]
  return Pieces(
    [_Piece(text, url=url, types=types, user=user)] if text is not None else [],
    emoji=emoji)
