...
class LiraRepo:
  ...
  def __init__(self, lira: Lira, lira_cat: str, lira_counter_id: str = None):
    ...
    self.liraCounterId = (lira_cat + '_id_counter'
                          if lira_counter_id is None else lira_counter_id)
  ...
  def newId(self) -> int:
    ...
    counter += 1
    self.lira.put(counter,
                  id=self.liraCounterId,
                  cat=LiraRepo.LIRA_COUNTER_ID_CATEGORY)
    self.lira.flush()
    return counter
