...
class User(TgState, LocatorStorage):
  ...
  async def _handleCallbackQuery(self, q: CallbackQuery) -> bool:
    asyncio.create_task(
      self.tg.answer_callback_query(q.id, 'Непонятно..')
    )
    return True
  ...
  async def handleNew(self):
    ...
    async def formEntered(data):
      article = self.master.newThing(Thing(
        rail=data[2],
        name=data[1],
    ...
    await self.setTgState(TgInputForm(
      tg=self.tg,
      chat=self.chat,
      ...
      fields=[
        ...
        TgInputField(
          ...
          validator=FunctionValidator(
            lambda o: ValidatorObject(success=False,
                                      error=P('Выберете категорию', emoji='fail'))
          ),
          on_field_entered=lambda _: None,
  ...
  async def handleSale(self):
    ...
    async def formEntered(data):
      ...
      for i in range(1, len(data[0])):
        isNotSold = data[0][i] if not self.master.removeThing(data[0][i]) else None
        if isNotSold: break
      if not isNotSold:
  ...
  async def handleTotal(self):
    if not self._checkTrusted():
      return
    percent = self.master.getMonthlyTotal() / self.config.rent() * 100
  ...
  def send(self, text):
    asyncio.create_task(
      send_message(tg=self.tg, chat=self.chat, text=text)
    )
  ...
  def validateName(o: ValidatorObject):
    ...
    if not (4 <= len(o.data) <= 64):
      o.success = False
      o.error = P('Название должно быть не меньше 4 символов и не больше 64 символов',
                  emoji='fail')
    return o
  ...
  def validateDescription(o: ValidatorObject):
    ...
    if len(o.data) < 10:
      o.success = False
      o.error = P('Описание вещи должно быть минимум 10 символов',
                  emoji='fail')
    return o
  ...
  def validateArticleIsExists(self, o: ValidatorObject):
    ...
    if isinstance(o.data, int):
      if not self.master.getThing(o.data): notArt.append(str(o.data))
    else:
      for art in o.data:
        if not self.master.getThing(art): notArt.append(str(art))
    if notArt:
      o.success = False
      o.error = P(f'{"Вещи" if len(notArt) == 1 else "Вещей"} {", ".join(notArt)} не припомню..', emoji='fail')
    return o
